...
public class CommandClan extends LMCommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		try {
			if (args.length > 0) {
				...
				if (c != null) {
					sendMessage(sender, c.getName() + " - " + c.getMemberRank(((EntityPlayer) sender).getUniqueID()));
					return;
				} else
					throw new IllegalArgumentException("playertrophy.command.noclan");
			}
		} catch (IllegalArgumentException | PlayerNotFoundException e) {
			sendErrorMessage(sender, Lang.translate(e.getMessage()));
		}
	}
	...
	private void handleLeave(ICommandSender sender, String[] params) {
		...
		if (c != null) {
			...
			sender.addChatMessage(new TextComponentString(
					String.format(Lang.translate("playertrophy.command.clanleave"), c.getName())));
		} else
			throw new IllegalArgumentException("playertrophy.command.noclan");
	}
	...
	private void handleList(ICommandSender sender) {
		...
		if (clans.size() > 0) {
			...
			for (Clan c : clans.values()) {
				sender.addChatMessage(new TextComponentString(c.getName() + " - " + c.getMembers().size() + " "
						+ Lang.translate("playertrophy.clan.members")));
			}
		} else
			throw new IllegalArgumentException("playertrophy.command.noclansfound");
	}
				...
				if (!PlayerTrophy.getInstance().getData().getClans().containsKey(clanName)) {
					...
					if (clan != null) {
						...
						sender.addChatMessage(new ChatComponentText(String.format(
								Lang.translate("playertrophy.command.clancreate"), clan.getName())));
					} else
						throw new IllegalArgumentException("playertrophy.command.clanexists");
				} else
					throw new IllegalArgumentException("playertrophy.command.clanexists");
			} else
				throw new IllegalArgumentException("playertrophy.command.invalidcommand");
		} else
			throw new IllegalArgumentException("playertrophy.command.alreadyinclan");
	}
	...
	private void handleDisband(ICommandSender sender, String[] params) {
		...
		if (c != null && c.delete(((EntityPlayer) sender).getUniqueID())) {
			sender.addChatMessage(new TextComponentString(
					String.format(Lang.translate("playertrophy.command.clandisband"), c.getName())));
			return;
		} else
			throw new IllegalArgumentException("playertrophy.command.invalidpermissions");
	}
		...
		if (params.length > 1) {
			...
			if (targetUuid != null && !c.hasMember(targetUuid) && player != null) {
				...
				player.addChatMessage(new TextComponentString(String
						.format(Lang.translate("playertrophy.command.invitenotification"), c.getName())));
				sender.addChatMessage(new TextComponentString(String.format(
						Lang.translate("playertrophy.command.inviteconfirmation"), player.getName())));
			} else
				throw new IllegalArgumentException("playertrophy.command.playernotfoundorinclan");
		} else
			throw new IllegalArgumentException("playertrophy.command.invalidcommand");
	}
			...
			if (c.hasMember(uuid)) {
				if (c.removeMember(uuid, ((EntityPlayer) sender).getUniqueID())) {
					PlayerTrophy.proxy.broadcastChatMessage(
							String.format(Lang.translate("playertrophy.command.bannednotification"),
									targetName, c.getName()));
					return;
				} else
					throw new IllegalArgumentException("playertrophy.command.invalidpermissions");
			} else
				throw new IllegalArgumentException("playertrophy.command.notamember");
		} else
			throw new IllegalArgumentException("playertrophy.command.invalidcommand");
	}
	...
	private void handleMembers(ICommandSender sender) {
		...
		if (c != null) {
			...
			while (mem.hasNext()) {
				...
				sender.addChatMessage(new TextComponentString(CommonProxy.getNameForUuid(m.getKey()) + " - "
						+ m.getValue().toString() + " - " + m.getKey().toString()));
			}
			return;
		}
		throw new IllegalArgumentException("playertrophy.command.noclan");
		...
		if (params.length > 1) {
			...
			if (!pdata.isInClan()) {
				if (clans.contains(x)) {
					pdata.accept(x);
					sender.addChatMessage(new TextComponentString(
							String.format(Lang.translate("playertrophy.command.clanjoined"), clanName)));
					return;
				} else
					throw new IllegalArgumentException("playertrophy.command.clannotfound");
			} else
				throw new IllegalArgumentException("playertrophy.command.alreadyinclan");
		} else
			throw new IllegalArgumentException("playertrophy.command.invalidcommand");
	...
	private void handleInvites(ICommandSender sender) {
		...
		if (clans.size() > 0) {
			...
			sendMessage(sender, Lang.translate("playertrophy.command.noinvites"));
	}
	...
	private void handleDefaultRank(ICommandSender sender, String[] params) {
		if (params.length > 2) {
			...
			sendMessage(sender, getClan(sender).getDefaultRank().toString());
	}
	...
	private void handleGrantRank(ICommandSender sender, String[] params) throws PlayerNotFoundException {
		if (params.length > 3) {
			...
			if (uuid == null)
				throw new IllegalArgumentException("playertrophy.command.playernotfound");
	...
	private void addClanRanks(ICommandSender sender, Collection<String> out, String start) {
		...
		for (Rank rank : ranks) {
			...
			if (doesStringStartWith(start, rankName))
				out.add(rankName);
		}
	}
	...
	private void addClanMembers(ICommandSender sender, Collection<String> out, String start) {
		...
		if (clan != null) {
			...
			for (Entry<UUID, Rank> e : list) {
				...
				if (doesStringStartWith(start, name))
					out.add(name);
			}
		}
	}
